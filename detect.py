...
def run(
        weights="E:/AXELIA/runs/train/exp3/weights/best.pt",  # model path or triton URL
...
def detect_image(image_np, conf_thres=0.25, iou_thres=0.45):
    ...
    if image_np.shape[2] != 3:
        raise ValueError("L'image doit avoir 3 canaux (RVB).")

    # Redimensionner l'image à la taille d'entrée du modèle (640x640)
    image_resized = cv2.resize(image_np, (640, 640))
    
    # Conversion de l'image en tensor PyTorch
    img = torch.from_numpy(image_resized).permute(2, 0, 1).float().unsqueeze(0)  # (1, 3, 640, 640)
    img = img.to(device) / 255.0  # Normalisation à [0, 1]
